...
JKY.Pieces = function() {
	...
	var my_filter		= 'jky-pieces-filter';
	var my_search_body	= 'jky-pieces-search-body';
	var my_layer		= 'jky-pieces-search';
	...
	function my_load_data_success(response) {
		...
		for(var i=0; i<my_rows.length; i++) {
			...
			my_html += '<tr onclick="JKY.Pieces.click_row(this, ' + my_row.id + ')">'
					+  '<td class="jky-pieces-search-name"			>' + my_row.name			+ '</td>'
					+  '<td class="jky-pieces-search-group"			>' + my_row.pieces_group	+ '</td>'
					+  '<td class="jky-pieces-search-composition"	>' + my_row.composition		+ '</td>'
					+  '</tr>'
					;
		}
	...
	function my_click_row(the_index, the_id) {
		var my_name = $(the_index).find('.jky-pieces-search-name').html();
		var my_parent = $(my_the_id).parent();
		$(my_parent).find('.jky-pieces-row-id'  ).val(the_id );
		$(my_parent).find('.jky-pieces-row-name').val(my_name);
		$(my_parent).find('.jky-pieces-row-name').click();
