...
/**
 * JKY.Transport - process al layer functions
 *
...
JKY.Transport = function() {
	...
	var my_specific		= 'is_transport';	//	selected transport type: Punho, Gola, Galao
	var my_order_by		= 'nick_name';
	var my_filter		= 'jky-transport-filter';
	var my_search_body	= 'jky-transport-search-body';
	var my_layer		= 'jky-transport-search';

	function my_display(the_id, the_specific) {
		my_the_id	= the_id;
		if (typeof the_specific	== 'undefined') {
			my_specific	= 'is_transport';
		}else{
	...
	function my_load_data_success(response) {
		...
		for(var i=0; i<my_rows.length; i++) {
			...
			my_html += '<tr onclick="JKY.Transport.click_row(this, ' + my_row.id + ')">'
					+  '<td class="jky-search-transport-name"	>' +				 my_row.nick_name		+ '</td>'
					+  '<td class="jky-search-transport-phone"	>' + JKY.fix_null	(my_row.phone		)	+ '</td>'
					+  '<td class="jky-search-transport-email"	>' +				 my_row.email			+ '</td>'
					+  '</tr>'
					;
		}
	...
	function my_click_row(the_index, the_id) {
		var my_name = $(the_index).find('.jky-search-transport-name').html();
