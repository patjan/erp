...
JKY.Cylinder = function() {
	...
	function my_load_data_success(response) {
		...
		for(var i=0; i<my_rows.length; i++) {
			...
			my_html += '<tr onclick="JKY.Cylinder.click_row(this, ' + my_row.id + ')">'
					+  '<td class="jky-Cylinder-search-name"			>' + my_row.name			+ '</td>'
					+  '<td class="jky-Cylinder-search-group"			>' + my_row.Cylinder_group	+ '</td>'
					+  '<td class="jky-Cylinder-search-composition"	>' + my_row.composition		+ '</td>'
					+  '</tr>'
					;
		}
