 ...
 *
 * 		$(my_parent).find('.jky-customer-row-id'  ).val(the_id );
 *		$(my_parent).find('.jky-customer-row-name').val(my_name);
 */
JKY.Customer = function() {
	...
	function my_click_row(the_index, the_id) {
		...
		if (my_dom_id.length == 0) {
			my_dom_id = $(my_parent).find('.jky-customer-row-id');
		}
		my_dom_id.val(the_id );

		var my_dom_name = $(my_parent).find('#jky-customer-name');
		if (my_dom_name.length == 0) {
			my_dom_name = $(my_parent).find('.jky-customer-row-name');
		}
