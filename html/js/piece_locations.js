...
*/
			...
			my_html  += ''
				...
				+ '<td class="jky-td-key"		><input class="jky-td-key"				value="' +				my_row.quotation_number	+ '" disabled	/></td>'
				+ '<td class="jky-td-key"		><input class="jky-td-key"				value="' +				my_row.order_number		+ '" disabled	/></td>'
				+ '<td class="jky-td-machine"	><input class="jky-checkin-machine"		value="' +				my_row.machine_name		+ '" disabled	/></td>'
				...
				+ '<td class="jky-td-pieces"	><input class="jky-reserved-pieces"		value="' +				my_pieces_reserved		+ '" disabled	/></td>'
				+ '</tr>'
				;
		}
	}
	JKY.set_html('jky-pieces-body', my_html);
	if (JKY.row.status ==  'Draft') {
		$('.jky-reserved-pieces').removeAttr('disabled');
		$('.jky-reserved-pieces').ForceIntegerOnly();
		$('.jky-reserved-pieces').on('change', function() {JKY.update_pieces()});
	}
	if (my_rows == '') {
//		JKY.insert_load();
	}
