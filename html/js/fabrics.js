...
//	$('#jky-thread-filter'	).KeyUpDelay(JKY.Thread.load_data);
	...
	JKY.set_side_active('jky-receiving-fabrics');
};
...
JKY.set_initial_values = function() {
	...
	JKY.set_html('jky-app-select'		, JKY.set_options(JKY.App.get_prop('select'), 'All', 'Active', 'Check In', 'Return', 'Check Out', 'Reprocess', 'Scrap', 'Ship Out'));
...
JKY.set_table_row = function(the_row) {
	...
	var my_loadout_number	= the_row.loadout_id   + ' <a href="#" onclick="JKY.open_new_tab(event, \'Sales/LoadOuts/'  + the_row.loadout_id   + '\')"><i class="icon-pencil"></i></a>';
	var my_html = ''
...
JKY.set_form_row = function(the_row) {
	...
	JKY.set_value	('jky-loadout-number'		,				 the_row.loadout_id			);
...
JKY.set_replace = function() {
	...
	JKY.set_html	('jky-status', JKY.set_options('', '', 'Active', 'Check In', 'Return', 'Check Out', 'Reprocess', 'Scrap', 'Ship Out'));
	JKY.set_value	('jky-barcode'				, '');
	JKY.set_value	('jky-loadout-number'		, '');
...
JKY.get_form_set = function() {
	var my_loadout_id	= JKY.get_value('jky-loadout-number');
	var my_product_id	= JKY.get_value('jky-product-id'	);
	var my_color_id		= JKY.get_value('jky-color-id'		);
		my_loadout_id	= (my_loadout_id	=== '') ? 'null' : my_loadout_id;
	...
	if (!JKY.is_empty(JKY.get_value('jky-loadout-id')))	{my_set += ', loadout_id= '	+ JKY.get_value('jky-loadout-number');}
	return my_set;
};
...
JKY.get_replace_set = function() {
	...
	if (!JKY.is_empty(JKY.get_value('jky-order-id'			)))	{my_set +=          ', order_id=  '	+ JKY.get_value('jky-order-id'			);}
	if (!JKY.is_empty(JKY.get_value('jky-product-id'		)))	{my_set +=        ', product_id=  '	+ JKY.get_value('jky-product-id'		);}
	if (!JKY.is_empty(JKY.get_value('jky-product-name'		)))	{my_set +=      ', product_name=\''	+ JKY.get_value('jky-product-name'		) + '\'';}
	if (!JKY.is_empty(JKY.get_value('jky-color-id'			)))	{my_set +=          ', color_id=  '	+ JKY.get_value('jky-color-id'			);}
	if (!JKY.is_empty(JKY.get_value('jky-color-name'		)))	{my_set +=        ', color_name=\''	+ JKY.get_value('jky-color-name'		) + '\'';}
	if (!JKY.is_empty(JKY.get_value('jky-loadout-id'		)))	{my_set +=        ', loadout_id=  '	+ JKY.get_value('jky-loadout-number'	);}
	if (!JKY.is_empty(JKY.get_value('jky-produced-by'		)))	{my_set +=       ', produced_by=\''	+ JKY.get_value('jky-produced-by'		) + '\'';}
	if (!JKY.is_empty(JKY.get_value('jky-number-of-pieces'	)))	{my_set +=  ', number_of_pieces=  '	+ JKY.get_value('jky-number-of-fabrics'	);}
