...
/**
 * JKY.Company - process al layer functions
 *
 * require:	JKY.Utils.js(JKY.display_confirm)
 *
 * 		$(my_parent).find('.jky-company-id'  ).val(the_id );
 *		$(my_parent).find('.jky-company-name').val(my_name);
 */
JKY.Company = function() {
	var my_the_id		= null;				//	external id that initiated the call
	var my_specific		= 'is_company';		//	selected company type: Punho, Gola, Galao
	var my_order_by		= 'nick_name';
	var my_filter		= 'jky-company-filter';
	var my_search_body	= 'jky-company-search-body';
	var my_layer		= 'jky-company-search';
	...
	function my_load_data_success(response) {
		...
		for(var i=0; i<my_rows.length; i++) {
			...
			my_html += '<tr onclick="JKY.Company.click_row(this, ' + my_row.id + ')">'
					+  '<td class="jky-search-company-name"	>' +				 my_row.nick_name	+ '</td>'
					+  '<td class="jky-search-company-phone">' + JKY.fix_null	(my_row.phone)		+ '</td>'
					+  '<td class="jky-search-company-email">' +				 my_row.email		+ '</td>'
					+  '</tr>'
					;
		}
	...
	function my_click_row(the_index, the_id) {
		var my_name = $(the_index).find('.jky-search-company-name').html();
		var my_parent = $(my_the_id).parent();

		var my_dom_id = $(my_parent).find('#jky-company-id');
		if (my_dom_id.length == 0) {
			my_dom_id = $(my_parent).find('.jky-company-id');
		}
		my_dom_id.val(the_id );

		var my_dom_name = $(my_parent).find('#jky-company-name');
		if (my_dom_name.length == 0) {
			my_dom_name = $(my_parent).find('.jky-company-name');
		}
