...
/**
 * JKY.HeadOffice - process al layer functions
 *
 * require:	JKY.Utils.js(JKY.display_confirm)
 *
 * 		$(my_parent).find('.jky-head-office-id'  ).val(the_id );
 *		$(my_parent).find('.jky-head-office-name').val(my_name);
 */
JKY.HeadOffice = function() {
	var my_the_id		= null;				//	external id that initiated the call
	var my_specific		= 'is_company';		//	selected head-office type: Punho, Gola, Galao
	var my_order_by		= 'nick_name';
	var my_filter		= 'jky-head-office-filter';
	var my_search_body	= 'jky-head-office-search-body';
	var my_layer		= 'jky-head-office-search';

	function my_display(the_id, the_specific) {
		my_the_id	= the_id;
		if (typeof the_specific	== 'undefined') {
			my_specific	= 'is_company';
		}else{
	...
	function my_load_data_success(response) {
		...
		for(var i=0; i<my_rows.length; i++) {
			...
			my_html += '<tr onclick="JKY.HeadOffice.click_row(this, ' + my_row.id + ')">'
					+  '<td class="jky-search-head-office-name"	>' +				 my_row.nick_name	+ '</td>'
					+  '<td class="jky-search-head-office-phone">' + JKY.fix_null	(my_row.phone)		+ '</td>'
					+  '<td class="jky-search-head-office-email">' +				 my_row.email		+ '</td>'
					+  '</tr>'
					;
		}
	...
	function my_click_row(the_index, the_id) {
		var my_name = $(the_index).find('.jky-search-head-office-name').html();
		var my_parent = $(my_the_id).parent();

		var my_dom_id = $(my_parent).find('#jky-head-office-id');
		if (my_dom_id.length == 0) {
			my_dom_id = $(my_parent).find('.jky-head-office-id');
		}
		my_dom_id.val(the_id );

		var my_dom_name = $(my_parent).find('#jky-head-office-name');
		if (my_dom_name.length == 0) {
			my_dom_name = $(my_parent).find('.jky-head-office-name');
		}
