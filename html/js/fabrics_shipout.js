...
JKY.start_program = function() {
	...
	JKY.App.set(
		{ object_name	: 'JKY.App'
		, program_name	: 'Fabrics Ship Out'
		, table_name	: 'Fabrics'
		, specific		: ''
		, select		: 'Check Out'
		, filter		: ''
		, display		: '20'
		, sort_by		: 'Fabrics.checkout_at'
		, sort_seq		: 'ASC'
		, sort_list		: [[3, 0]]
...
//	$('#jky-action-close'		).click( function() {JKY.App.close_row(JKY.row.id);});
	...
	JKY.set_side_active('jky-fabrics-shipout');
};
...
JKY.process_barcode_success = function(response) {
	...
	if (my_row) {
		...
		if ($('#jky-pieces-table-body td:contains("' + my_barcode + '")').length > 0) {
			...
			var my_status_class = '';
			if (my_row.status != 'Check In'		//	Pat Jan temporary fix
			&&  my_row.status != 'Check Out'
