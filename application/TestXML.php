...
*/
		...
		$my_xml = new SimpleXMLElement($xml_data);
//		$my_json = json_encode($my_xml);
//		echo $my_json;
/*
		foreach($my_xml as $my_row) {
			$my_json = json_encode($my_row);
			echo  str_replace( ",", "\n,"
				, str_replace( "{", "\n{"
				, str_replace( "}", "\n}", $my_json )));
		}
*/
		...
		function parse_xml($the_level, $the_xml) {
			$my_level = $the_level . "\t";
			$my_text  = '';
			foreach($the_xml as $key => $value) {
				$my_text .= "\n" . $my_level . ' ' . $key . ' : ';
				if (count($value)>0) {
					$my_text .= parse_xml($my_level, $value);
				}else{
					$my_text .= $value;
//echo $my_text;
				}
		...
		$my_all = parse_xml($my_level, $my_xml) . "\n";
		echo $my_all;
//		var_dump($my_xml);
