...
function JKY_invoice_customer($the_id) {
	...
	$my_count = 0;
	return JKY_echo_count($my_count);
}
...
log_sql('Transport', 'INSERT', $my_id);
	...
	foreach($loadouts as $loadout) {
		$loadsales = db_get_rows('LoadSales',  'loadout_id=' . $loadout['id']);
		foreach($loadsales as $loadsale) {
			$pieces = db_get_rows('Pieces'	, 'loadsale_id=' .$loadsale['id']);
